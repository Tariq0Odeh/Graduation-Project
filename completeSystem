# -*- coding: utf-8 -*-
"""GradProject_(MultipleResourceChanges).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1MgXB_5__jfmtXaCWdxBYh-DC4zA0hdlQ
"""
...
class Environment(gym.Env):
    ...
    def reset(self):
        ...
        for i in range(numOfSubnetworks):
            newCDF = np.zeros(16)
            beta = 100
            # newObservation = np.concatenate((beta, newCDF))
            newObservation = np.insert(newCDF, 0, beta)
    ...
    def step(self, action, subnetworks, episode):
        ...
        for i in range(numOfSubnetworks):
            ...
            newResource = subnetworks[i].r[0] + mapAction(action)
...
if __name__ == '__main__':
    agent = Agent(alpha=0.0001, beta=0.001, input_dims=[numOfSubnetworks * (16 + 1)], gamma=0.99,
                  n_actions=3, layer1_size=32, layer2_size=64)
    ...
    subnetwork_loss = Subnetwork(voip_simulation_loss, 0.25)
    subnetwork_delay = Subnetwork(voip_simulation_delay, 0)
    ...
    for i in range(num_episodes):
        ...
        while not done:
            ...
            while True:
                ...
                resources_array.append(subnetworks[0].r[0])

                observation = observation_

                rbs = subnetworks[0].r[0]
                mappedAction = mapAction(action)
                if (rbs == 8 and mappedAction == 1 and lastAction == 1) or (rbs == 1 and mappedAction == -1 and lastAction == -1) or (mappedAction == 0 and lastAction == 0):
...
plt.plot(resources_array)
plt.savefig('resources_array.png', format='png')
