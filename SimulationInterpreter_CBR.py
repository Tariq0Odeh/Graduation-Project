...
folder_path = 'cbr'
json_path = 'cbr_d30sec_results.json'
...
def readResults(csv_file, numOfUE):
    ...
    for i in range(1, int(numOfUE) + 1):
        ...
        filtered_loss = filtered[(filtered['name'].str.contains('cbrFrameLoss'))]
        filtered_loss.loc[:, 'value'] = pd.to_numeric(filtered_loss['value'])
        data_loss.append(np.mean(filtered_loss.value))

        filtered_jitter = filtered[(filtered['name'].str.contains('cbrJitter'))]
